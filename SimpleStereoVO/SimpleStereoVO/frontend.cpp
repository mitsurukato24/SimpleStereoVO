...
namespace simple_vo
{
	...
	bool Frontend::AddFrame(simple_vo::Frame::Ptr frame) 
	{
	...
	bool Frontend::Track()
	{
		...
		auto t2_track = std::chrono::steady_clock::now();
		auto time_used_track =
			std::chrono::duration_cast<std::chrono::duration<double>>(t2_track - t1_track);
		LOG(INFO) << "track cost time: " << time_used_track.count() << " seconds.";
